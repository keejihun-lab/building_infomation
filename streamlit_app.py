...
def fetch_building_info(dong: str, gu: str, jibeon: str, ho: str):
    ...
    def _fetch_expos(ho_nm_val):
        p = {**base_params, "hoNm": ho_nm_val} if ho_nm_val else base_params
        items = _call("getBrExposPubuseAreaInfo", p)
        if ho_nm_val and items:
            filtered = [e for e in items if str(e.get("hoNm", "")).strip().rstrip("호") == ho_clean]
            if filtered:
                return filtered
        return items

    expos_items = _fetch_expos(ho_clean)
    if not expos_items and ho_clean:
        expos_items = _fetch_expos(ho_clean + "호")
...
with tab_listing:
    ...
    with col2:
        ...
        rooms = st.text_input("방/욕실 개수", value="1/1", key="ti_rooms")
